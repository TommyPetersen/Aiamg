...
public class Util3d{
    ...
    // *** Projection ***
    public static <T> ArrayList<T> projectGeoms(ArrayList<T> geoms, double projectPlaneValue, ProjectionType projectionType) throws Exception{
        ...
        while (geomIterator.hasNext()){
            ...
            projectedGeoms.add(projectGeom(geom, projectPlaneValue, projectionType));
        }
    ...
    @SuppressWarnings("unchecked") // Suppress warnings on cast from generic to specific type.
    public static <T> T projectGeom(T geom, double projectPlaneValue, ProjectionType projectionType) throws Exception{
        T projectedGeom;

        switch (geom.getClass().getName()){
        case "Aiamg.Point3D":
            projectedGeom = (T) projectPoint((Point3D) geom, projectPlaneValue, projectionType);
            break;
        case "Aiamg.Line3D":
            projectedGeom = (T) projectLine((Line3D) geom, projectPlaneValue, projectionType);
            break;
        case "Aiamg.Triangle3D":
            projectedGeom = (T) projectTriangle((Triangle3D) geom, projectPlaneValue, projectionType);
            break;
        default:
            throw new Exception("Class type '" + geom.getClass().getName() + "' is not supported");
        }
    ...
    public static Triangle3D projectTriangle(Triangle3D spaceTriangle, double projectPlaneValue, ProjectionType projectionType) throws Exception{
        Triangle3D projectedTriangle = new Triangle3D(projectPoint(spaceTriangle.P0, projectPlaneValue, projectionType),
                                                      projectPoint(spaceTriangle.P1, projectPlaneValue, projectionType),
                                                      projectPoint(spaceTriangle.P2, projectPlaneValue, projectionType));
        return projectedTriangle;
    }

    public static Line3D projectLine(Line3D l, double projectPlaneValue, ProjectionType projectionType) throws Exception{
        return new Line3D(projectPoint(l.P0, projectPlaneValue, projectionType),
                          projectPoint(l.P1, projectPlaneValue, projectionType));
    }
    
    public static Point3D projectPoint(Point3D p, double projectPlaneValue, ProjectionType projectionType) throws Exception{
        double x = p.x;
        double y = p.y;
        double z = p.z;
